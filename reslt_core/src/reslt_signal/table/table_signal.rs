...
where
    T: 'static + Serialize + Eq + Clone + FieldAccessible + Debug,
{
    let state = use_table(fetch_fn, cols, sort_state);
    use_context_provider(|| state.clone());
    state
}

/// Returns the table context for accessing table state
pub fn use_table_context<T>() -> UseTable<T>
where
    T: 'static + Serialize + Eq + Clone + FieldAccessible + Debug,
{
    use_context::<UseTable<T>>()
}
...
where
    T: 'static + Serialize + Eq + Clone + FieldAccessible + Debug,
{
    pub prop_data: Signal<PropData<T>>,
    pub prop_col: Signal<PropCol<T>>,
    pub sort_state: Signal<SortState>,
    pub page_state: Signal<PageState>,
